...
Bullet::Bullet(int imageID, int maskID)
{
...
Bullet::Bullet(const char* szImageFile, const char* szMaskFile)
{
...
Bullet::Bullet(const char* szImageFile, COLORREF crTransparentColor)
{
	mhImage = (HBITMAP)LoadImage(g_hInst, szImageFile, IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_LOADFROMFILE);
...
void Bullet::setBackBuffer(const BackBuffer* pBackBuffer)
{
	mpBackBuffer = pBackBuffer;
	if (mpBackBuffer)
	{
		DeleteDC(mhBulletDC);
		mhBulletDC = CreateCompatibleDC(mpBackBuffer->getDC());
	}
}
...
void Bullet::drawMask()
{
	...
	// The position BitBlt wants is not the Bullet's center
	...
	HGDIOBJ oldObj = SelectObject(mhBulletDC, mhMask);

	// Draw the mask to the backbuffer with SRCAND. This
	// only draws the black pixels in the mask to the backbuffer,
	// thereby marking the pixels we want to draw the Bullet
	// image onto.
	BitBlt(hBackBufferDC, x, y, w, h, mhBulletDC, 0, 0, SRCAND);

	// Now select the image bitmap.
	SelectObject(mhBulletDC, mhImage);
	...
	BitBlt(hBackBufferDC, x, y, w, h, mhBulletDC, 0, 0, SRCPAINT);

	// Restore the original bitmap object.
	SelectObject(mhBulletDC, oldObj);
}

void Bullet::drawTransparent()
{
	if (mpBackBuffer == NULL)
		return;
...
AnimatedBullet::AnimatedBullet(const char* szImageFile, const char* szMaskFile, const RECT& rcFirstFrame, int iFrameCount)
	: Bullet(szImageFile, szMaskFile)
{
...
void AnimatedBullet::SetFrame(int iIndex)
{
	// index must be in range
	assert(iIndex >= 0 && iIndex < miFrameCount && "AnimatedBullet frame Index must be in range!");

	mptFrameCrop.x = mptFrameStartCrop.x + iIndex * miFrameWidth;
	mptFrameCrop.y = mptFrameStartCrop.y + iIndex * miFrameHeight;
}

void AnimatedBullet::draw()
{
	if (mpBackBuffer == NULL)
		return;
	...
	// thereby marking the pixels we want to draw the Bullet
	// image onto.
	BitBlt(hBackBufferDC, x, y, w, h, mhBulletDC, mptFrameCrop.x, mptFrameCrop.y, SRCAND);
	...
	// will only draw the image onto the pixels that where previously
	// marked black by the mask.
	BitBlt(hBackBufferDC, x, y, w, h, mhBulletDC, mptFrameCrop.x, mptFrameCrop.y, SRCPAINT);
